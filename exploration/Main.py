...
stations = compute_possible_stations.get_stations(obj, h, w, predict_func, initial, visdist[0], min_distance)
print stations

moveobstacles = [147] #Initial state of moving obstacle(s)
gwg = Gridworld(filename,targets=[stations],initial=initial,moveobstacles=moveobstacles) #Generate gridworld class
gwg.render()
...
vel = [5] #No of states the agent can move in one timestep
print 'Writing input file...'
invisibilityset = []
outfile = 'Example1.json'
infile = 'Example1'
...
for n in range(gwg.nagents):
    ...
    Salty_input.write_to_slugs_part_dist(infile,gwg,initial[n],moveobstacles[n],iset,stations,vel[n],visdist[n],
...
simulateController.userControlled_partition_dist([outfile],gwg,pg,moveobstacles,allowed_states,invisibilityset)